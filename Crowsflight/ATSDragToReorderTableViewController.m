...
- (void)longPressRecognized {
	...
	[UIView animateWithDuration:0.2 delay:0 options:(UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionCurveEaseInOut) animations:^{
...
- (void)shuffleCellsOutOfWayOfDraggedCellIfNeeded {
	...
	if (rowToMoveTo != nil && !(rowToMoveTo.section == self.indexPathBelowDraggedCell.section && rowToMoveTo.row == self.indexPathBelowDraggedCell.row)) {
		 ...
		 */
        if(formerBlankIndexPath.section== rowToMoveTo.section){
...
- (CGFloat)distanceOfCellCenterFromEdge {
	...
	if (yOffsetOfDraggedCellCenter > heightOfTableView/2.0) {
		 ...
		 */
		CGFloat paddingAgainstBottom = 22.0;
...
- (CGFloat)autoscrollDistanceForProximityToEdge:(CGFloat)proximity {
	 ...
	 */
    CGFloat maxAutoscrollDistance = 8.0;
    if ( [[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad )
        maxAutoscrollDistance = 12.0;
...
- (NSArray *)addShadowViewsToCell:(UITableViewCell *)selectedCell {
	 ...
	 */
	if (selectedCell.selectedBackgroundView == nil) return nil;
...
//	CGFloat heightOfViews = 20; // make it enough space to show whole shadow
...
//	[selectedCell bringSubviewToFront:belowShadowView];
    ...
    selectedCell.layer.shadowOffset = CGSizeMake(0, 2);
    selectedCell.layer.shadowRadius = 8;
    selectedCell.layer.shadowOpacity = 0.8;
